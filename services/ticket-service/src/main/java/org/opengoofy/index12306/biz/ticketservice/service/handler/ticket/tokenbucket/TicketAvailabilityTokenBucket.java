...
public final class TicketAvailabilityTokenBucket {
    ...
    public boolean takeTokenFromBucket(PurchaseTicketReqDTO requestParam) {
        ...
        TrainDO trainDO = distributedCache.safeGet(
                ...
                ADVANCE_TICKET_DAY,
                TimeUnit.DAYS);
        // 获取这段旅程列车经停站之间的数据集合，因为一旦失效要读取整个列车的令牌并重新赋值
        // 比如列车总共 A-F的路程，当前旅客要求路程 B-E，需要路程集合为返回 B-C、B-D、B-E、C-D、C-E、D-E
        // 此路程集合用于票数扣减，因为B-E路程包含了这个集合的所有票，需连带扣减
        ...
        if (!hasKey) {
            ...
            try {
                ...
                if (!hasKeyTwo) {
                    ...
                    //路程以及子路程加令牌，令牌组成：KEY[列车id+起始点+等级],VALUE[座位数量]
        ...
        //返回乘车人需求票数集合，比如一次购买包含 2张二等票，3张一等票
        ...
        //执行LUA脚本
        //传入参数：LUA脚本对象，lua的hashkey集合，这趟列车总共的票集合，乘车人总共需求票集合
